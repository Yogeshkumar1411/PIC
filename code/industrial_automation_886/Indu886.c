...
/*Name: Yogesh
Date: 03-07-2022
Concept: Generator maintenance system.*/
#include<pic.h>
#include<math.h>
#include"lcd.h"
#define EB_On RA0//EB mode input
#define Generator_On RA1//Generator mode input
#define Heat_danger RC1//Fire alarm while fire hazard
#define Shift_man_alarm RC0//Shift management Siren
#define EB_LED_on RC2//indication of EB mode
#define Gen_LED_on RC5//indication of Generator mode
#define Diesel_1 RA2//diesel level 1 litre
#define Diesel_2 RA3//diesel level 0.5 litre
#define Reset RA4//Reset button the flags for GSM purpose
...
{
		auto int Gen_a,Gen_b,Gen_c,Gen_d,Gen_e,Gen_f;//Variables for generator's run time at present
		lcd_cmd(0,0,0XC0);
		string("Gen.run:");
		Gen_a=variable/1000;
		Gen_b=variable%1000;
...
{	
	if(flag1==1)
	{
			...
			{
				trans("Generator diesel reached low level 2 liter");
			}
			if(cmd==3)//For 0.5 litre
			{
				trans("Generator diesel reached very low level 1 liter");
			}
			new();
			delay(10000);
			tran(0X1A);
...
void adc()//PT100 adc conversion
{
	auto int vari1;
	auto float temp;
	auto int high,low,sensorvalue;//For the PT100 samplings
	auto int temp_a,temp_b,temp_c,temp_d,temp_e,temp_f;//PT100 variables
